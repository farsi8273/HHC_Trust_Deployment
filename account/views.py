...
def generate_pdf(name,cert_id,amount,pay_id,phone_no,pan_card,email,pay_mode,ip, date):
    context = {'name': name, 'no':cert_id,'date':date,'amount':amount,'id':pay_id,'pan_card_no':pan_card,'gmail':email,'phone_no':phone_no,'pay_mode':pay_mode,'ip':ip}
    ...
    pdf = pdfkit.from_string(rendered_html, output_path,configuration=config)
    response = HttpResponse(pdf, content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename=f"{i}.pdf"'
    return output_path

def sendMail(name,cert_id,amount,pay_id,email,phone_no,pan_card,pay_mode,ip, date):
    pdf = generate_pdf(name,cert_id,amount,pay_id,phone_no,pan_card,email,pay_mode,ip, date)
    date1= date
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if donation.Certificate_80G:
                ...
                pdf = sendMail(donation.name,id_keyword,donation.amount,donation.pay_id,donation.email,donation.phone_number,donation.pan_card,donation.pay_mode,ip, donation.donation_date)
...
def donors_list(request):
    donations = Certificate_80g.objects.all()
    donation_lst = []
    for donation in donations:
        donor_dict = {}
        donor_dict['donor_id'] = donation.donater.id
        donor_dict['name'] = donation.donater.name
        donor_dict['email'] = donation.donater.email
        donor_dict['campaigns'] = donation.donater.campaigns
        donor_dict['amount'] = donation.donater.amount
        donor_dict['donation_date'] = donation.donater.donation_date
        donor_dict['url'] = request.build_absolute_uri('/') + '/'.join(donation.pdf_file.url.split('/')[4:])
